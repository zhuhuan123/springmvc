...
<div id="roids" style="display: none"> <!-- style="{display: none}" -->
	<form method="post" id="fowid" action="saveuserrole">
		<input type="hidden" name="userid" id="usid">
		<table>
		...
		<tr>
		<td><input type="button" id="savid"  value="提交"></td>
		<td><input type="reset" value="重置"></td>
		</tr>
		</table>
	</form>
...
$(function(){
	$("#savid").click(function(){
		$.post("saveuserrole",$("#fowid").serialize(),function(data){
...
function roleAndUser(){
	var i = grid.getSelected();
	if(i==null){
		alert("选中一行");
		return ;
	}
	$("#usid").val(i.id);
	var t="" ;
	$.post("getRolesAll",function(data){
		for(var i=0;i<data.length;i++){
			t=t+"<tr><td>"+data[i].name+"</td><td><input type='checkbox' name='rol' value='"+data[i].id+"' ></td></tr>";
		}
		$("#roles").html(t);
	});
	if(dial==null){
		dial = $.ligerDialog.open({
			target: $("#roids"),
			height: 260,
			width: 400,
			title: "添加",
